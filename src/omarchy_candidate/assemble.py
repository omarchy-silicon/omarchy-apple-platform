...
from .models import ArtifactReader, CandidateAssemblyInput, CandidateAuthority, CandidateManifest, _AuthorityCapability, _MANIFEST_TOKEN, _utc_time, digest_bytes, digest_value, _freeze
...
def assemble_candidate(
    ...
    *,
    ...
    try:
        checked_time = _utc_time(verification_time, "$.verification_time")
    ...
    for kind, record, digest, subject in checks:
        ...
        try:
            ...
            raise CandidateAssemblyError("AUTHORITY_UNTRUSTED", f"$.{kind}", "authority returned an invalid trusted receipt") from error
    ...
    artifact_rows = tuple({"artifact_id": artifact_id, **item} for artifact_id, item in sorted(assembly.package["artifacts"].items()))
    ...
    if set(indexed) != {item["artifact_id"] for item in artifact_rows}:
    ...
    package_digests = {item["content_digest"] for item in artifact_rows}
