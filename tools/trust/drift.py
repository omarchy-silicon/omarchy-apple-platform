...
def main() -> int:
    ...
    if manifest.get("format") != "omarchy-trust-fixtures/v1" or not isinstance(vectors, list) or len(vectors) != 18:
    ...
    for path in [*anchor_paths, ROOT / "policy/trust/role-policy.json", ROOT / "policy/trust/trust-bundle.schema.json", *[ROOT / "fixtures/trust" / name for name in names]]:
