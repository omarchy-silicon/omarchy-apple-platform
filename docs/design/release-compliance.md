...
The path mapping is lossless and fail-closed: the original F-02 `JsonPath` is preserved verbatim, while the local `JsonPointer` always retains the import root, for example `/f02_imports/platform-manifest~1v1`. F-02 array members are never localized by position. Instead, an array finding carries `array_container_pointer` plus a stable `subject_key` and, when the source contract supplies one, a stable `source_identity`; the container pointer is the only local pointer. A source path that cannot be represented without an array index is `F06-F02-PATH-UNREPRESENTABLE` with the unchanged source `JsonPath` and the import/container pointer, never a shortened, wildcard, numeric, or whole-request path.
...
F06_CODES = {
  ...
  F06-IDENTITY-DIGEST-MISMATCH, F06-PROVENANCE-MISSING, F06-PROVENANCE-CONFLICT, F06-SOURCE-AUTHORITY-MISSING, F06-SOURCE-AUTHORITY-REMOVED, F06-SOURCE-AUTHORITY-REORDERED, F06-SOURCE-AUTHORITY-UNLISTED, F06-SOURCE-ACQUISITION-PRECONSENT, F06-SOURCE-NETWORK-POSTCONSENT, F06-SOURCE-CONTENT-MISMATCH, F06-LICENSE-MISSING, F06-LICENSE-DENIED, F06-LICENSE-CHOICE-AMBIGUOUS, F06-NOTICE-MISSING, F06-SOURCE-OFFER-INCOMPLETE, F06-SOURCE-OFFER-UNSIGNED, F06-SOURCE-OFFER-UNAVAILABLE, F06-REDISTRIBUTION-PROHIBITED, F06-EXPORT-RESTRICTED, F06-SECURITY-UNRESOLVED, F06-COUNSEL-CONTRADICTION, F06-BRANDING-UNRESOLVED, F06-OWNER-DENIED,
  F06-POLICY-UNKNOWN, F06-POLICY-UNAVAILABLE, F06-POLICY-PREDICATE-FALSE, F06-POLICY-DEPTH-EXCEEDED, F06-POLICY-CYCLE, F06-POLICY-DUPLICATE, F06-POLICY-DIGEST-MISMATCH, F06-HANDOFF-DUPLICATE, F06-HANDOFF-CYCLE, F06-PRODUCT-INTEGRATION-MISSING,
...
`FailureCode` is the disjoint union of one exact member of `F06_CODES` or one exact `F06-F02-SOURCE-<source_code>` value. The local registry assigns every `F06_CODES` member exactly one phase and exactly one decision class; the same code can never mean `HOLD` in one row and `REJECT` in another. In particular, `F06-SOURCE-AUTHORITY-MISSING` is the only pre-consent missing-authority `HOLD`, while removed, reordered, unlisted, and pre-consent acquisition violations are distinct `REJECT` codes and post-consent network acquisition is `F06-SOURCE-NETWORK-POSTCONSENT` `REJECT`. `F06-P0`/`F06-P1` parse and grammar codes are `REJECT`; `F06-P2` canonical and preimage codes are `REJECT`; `F06-P3` trust and binding codes are `HOLD` only for unavailable foundations, otherwise `REJECT`; `F06-P4` identity and scope codes are `REJECT`; `F06-P5` inventory and source contradictions are `REJECT`, with only `F06-SOURCE-AUTHORITY-MISSING` `HOLD`; `F06-P6` provenance and artifact contradictions are `REJECT`, with only absent required provenance `HOLD`; `F06-P7` decision denial/contradiction is `REJECT` and missing authority is `HOLD`; `F06-P8` event, projection, and retention codes use their one registered class; `F06-P9` promotion and handoff codes are `REJECT`; and `F06-P10` report, clean-checkout, tracked-artifact, secret, conflict, path, placeholder, fixture, and tooling codes are `REJECT` except `F06-TOOLING-BLOCK`, which is a non-pass `TOOLING_BLOCK` result. The registered class table and fixture manifest are checked for exact agreement; a code outside this disjoint registry, duplicate code definition, conflicting class, or unassigned path family is a contract failure.
...
Finding = {
  fixture_id: LowerAsciiToken | null,
  code: FailureCode,
  path: JsonPointer,
  array_container_pointer: JsonPointer | null,
  phase: F06Phase,
  subject_key: LowerAsciiToken,
  predicate: LowerAsciiToken,
  action: LowerAsciiToken,
  expected_result: "PASS" | "ADMIT" | "HOLD" | "REJECT" | "NOT EXECUTABLE" | "TOOLING_BLOCK",
  expected_result_digest: Digest | null,
...
The normalized fixture/result record is closed and total: `FixtureResult = { fixture_id, baseline_digest, mutation: { predicate, action, subject_key, array_container_pointer }, expected: { decision, code, path, phase, result_digest }, observed: { decision, code, path, phase, result_digest } | null, current }`. `predicate` names the one changed condition, `action` names the one operation that plants it, `expected_result_digest` is required when the future result has a digest oracle and is explicitly `null` for a design-only row, and `subject_key` disambiguates every array member without an index. The unique result key is `(fixture_id, subject_key, code, path, phase)`; duplicate code/path/phase values across different subjects are valid only when this complete key remains unique and the expected result is identical.

The total evaluator first parses and bounds the entire request, canonicalizes every available closed object, verifies imported F-02/F-03 bindings, verifies local identity and scope, checks inventory and source closure, checks provenance, checks decisions and policy, checks event/projection/retention dependencies, verifies the F-07 handoff, and canonicalizes the result. It records every independently observable finding but chooses the primary finding by the stable key `(phase_ordinal, path UTF-8 bytes, subject_key UTF-8 bytes, code UTF-8 bytes, source_phase, source_json_path)`. The output finding order and result digest are therefore invariant under permutation of unordered request maps, handoff arrays, and reference input order. A semantically ordered array is itself validated for its declared order; a permutation that changes a required order is a different invalid input, not evaluator-order dependence. No stop-first, warning-success, first-map-member, last-map-member, deduplicate-and-continue, or input-order winner exists. The precedence oracle is separate from artifact fixtures: it sorts this key over a synthetic list of independent finding records and compares the ordered key/result digest; it never mutates one artifact with multiple faults and never uses a broad root pointer.
...
Each union branch has exactly the fields shown, `all` and `any` have at least one child, `in-digest-set` has at least one digest, child arrays are sorted by canonical child bytes, and the total recursive depth is at most `max_condition_depth = 16` with no more than `max_array_items` condition nodes. `condition_key` is the derived `sha256(ASCII("omarchy-f06-policy-condition/v1") || 0x00 || JCS(PolicyCondition))` used only for array ordering and duplicate detection; aliases are not values and a repeated key on the recursion stack is a cycle. A condition reads only the named closed-field JSON Pointer and compares a digest or presence bit; it cannot carry raw values, arbitrary expressions, code, URLs, maps, a field selector outside the request/result records, or a default allow. `policy_digest = sha256(ASCII("omarchy-f06-policy/v1") || 0x00 || JCS(F06Policy without policy_digest))`; true evaluates to `allow`, false to `reject`, and unavailable required input to `hold`. Unknown kind is `F06-POLICY-UNKNOWN`, unavailable fact is `F06-POLICY-UNAVAILABLE`, false is `F06-POLICY-PREDICATE-FALSE`, excessive depth is `F06-POLICY-DEPTH-EXCEEDED`, a recursive/alias cycle is `F06-POLICY-CYCLE`, a duplicate key is `F06-POLICY-DUPLICATE`, and an invalid digest reference is `F06-POLICY-DIGEST-MISMATCH`; each is reported at the earliest exact condition container pointer. Other malformed shape, empty logical array, malformed path, or extra field remains `F06-INPUT-MALFORMED`.

F-06 uses the exact future F-02 grammar for `Digest`, `DocumentId`, `ProjectId`, `RepositoryId`, `SliceId`, `ArtifactId`, `ComponentId`, `PolicyId`, `ActorId`, `AccountId`, `Timestamp`, `Version`, `ApiVersion`, `UUID`, `JsonPath`, `JsonPointer`, `Channel`, `SourceReferenceId`, and `EvidenceId`; imported F-02 `Operation` values retain the exact F-02 enum. F-06-specific identifiers use lowercase ASCII prefixed tokens and never use free-form names. `JsonPointer` is restricted by the imported F-02 lock: no numeric array-index segment is permitted, and every array member is localized only by its non-index container pointer plus stable subject/source identity. URLs are immutable HTTPS locations with no credentials, query, fragment, IP-literal authority, or mutable selector. Paths are normalized relative paths; absolute paths, `~`, `$VAR`, command substitutions, globs, `..`, backslashes, and placeholders are rejected.
...
The installer must complete the exact sequence: resolve the trusted installer-plan reference; enumerate every direct and transitive input; verify that each authority is present, unique, allowlisted, and in the declared order; acquire only through the eight allowlisted authorities before consent; verify signature, selector, provenance, scope, digest, size, and freshness; create a content-addressed cache object and receipt; verify the inventory closure; present the exact plan, closure, rollback boundary, and consent scope; obtain final consent; and only then cross the destructive boundary. F-06 only verifies the resulting handoff and never performs this sequence. Missing, removed, reordered, or unlisted authority metadata and any acquisition before consent each have a distinct registered failure code; no source closure state is inferred from a nearby authority or from a post-consent attempt.
...
`NetworkPolicyEvidence` is complete only when its `allowed_endpoint_set_digest` covers the pre-consent allowlist, its denied endpoint and operation sets cover every post-consent resolver, and its evidence digest is recomputed from the observed deny result and exact consent/scope tuple. After final consent, network acquisition is denied: DNS, HTTP, Git, package resolution, mirror selection, credential lookup, and resolver fallback cannot run. Missing required authority evidence is `HOLD` `F06-SOURCE-AUTHORITY-MISSING` before mutation. Removed, reordered, unlisted, and pre-consent acquisition violations are `REJECT` with their respective registered codes. Mutable, stale, unapproved, cross-target, digest-mismatched, direct-device-unreceipted, or network-dependent input is `REJECT` at its exact source subject. A network attempt after consent is `REJECT` `F06-SOURCE-NETWORK-POSTCONSENT` at `/source_closure/network_policy/mode`; it cannot become a warning-success.
...
The command vector is a valid JSON value containing exactly 17 entries, each with one literal `argv` array. It contains no shell conditionals, shell pipelines, JSON-like pseudo-arrays, escaped globs, ellipses, environment fallback, or unbounded discovery. Every entry is bound to this exact execution context, and the context digest is included in the report:

```text
ExecutionContext = {
  cwd: ".",
  environment: { PATH: "/usr/bin:/bin", LANG: "C.UTF-8", LC_ALL: "C.UTF-8", TZ: "UTC", PYTHONHASHSEED: "0" },
  inherited_environment: false,
  stdin: "closed-empty",
  stdout: "capture-separate; max_bytes=8388608; overflow=REJECT",
  stderr: "capture-separate; max_bytes=8388608; overflow=REJECT",
  timeout_seconds: 120,
  timeout_signal: "SIGTERM-then-SIGKILL-after-5-seconds",
  umask: "0077",
  output_size: "any stream over max_bytes is F06-BOUND-EXCEEDED; no truncated success",
  repository_root: "runner-resolved checkout root only"
}
```

The runner resolves `cwd` to the checkout root, sets exactly the listed environment, closes stdin, captures stdout and stderr separately, applies the timeout and signal policy, sets umask before spawning, and rejects output overflow. No host environment, current directory, locale, timezone, inherited descriptor, signal default, or shell wrapper is observable by a command.
...
[
  ...
  ["rg", "--line-number", "--hidden", "--glob", "!.git/**", "<<<<<<<", "schemas", "bindings", "src", "tests", "reports", ".github"],
  ["rg", "--line-number", "--hidden", "--glob", "!.git/**", "private_key|password|bearer", "schemas", "bindings", "src", "tests", "reports", ".github"],
  ["rg", "--pcre2", "--line-number", "--hidden", "--glob", "!.git/**", "(^|[^A-Za-z0-9_])/(?!/)|file://|\\.\\./|(^|[^A-Za-z0-9_])~", "schemas", "bindings", "src", "tests", "reports", ".github"],
  ["rg", "--line-number", "--hidden", "--glob", "!.git/**", "UNRESOLVED_MARKER|PLACEHOLDER_DIGEST|PLACEHOLDER_PATH", "schemas", "bindings", "src", "tests", "reports", ".github"],
...
The command vector contains exactly 17 command entries and exactly 17 `argv` arrays: four repository-state checks, eight future module invocations, four forbidden-content scans covering `schemas`, `bindings`, `src`, `tests`, `reports`, and `.github`, and one tracked-deliverable enumeration. The command-vector digest covers the literal arrays plus `ExecutionContext`; a command entry without the context binding is invalid.
...
The fixture manifest is closed to the hostile IDs below and the positive IDs below. Each fixture has one canonical baseline, exactly one mutation represented by a predicate and action, one stable subject identity, one expected decision, one exact code, one exact non-index pointer, one exact F-06 phase, and one expected result evidence record. Every compact table row expands deterministically to `predicate = single-mutation:<fixture-id>`, `action = replace-one-baseline-member`, `subject_key = subject:<fixture-id>`, and `expected_result_digest = null` unless a future vector explicitly supplies a digest. Every row is currently `NOT IMPLEMENTED`; a prose model is not a runnable fixture. The runner must construct every row, plant only its mutation, and compare predicate, action, subject key, code, path, phase, decision, and result digest. No artifact fixture may combine mutations. Array rows use the container pointer and stable subject identity; they never add an index segment.

The separate precedence-key/order oracle is not a fixture and is excluded from the hostile count. It contains independent finding records with `(phase_ordinal, path UTF-8 bytes, subject_key UTF-8 bytes, code UTF-8 bytes, source_phase, source_json_path)`, sorts that key, and asserts a stable ordered result digest under permutation. It does not create a multi-fault artifact, does not mutate a whole-request root, and cannot satisfy any fixture or implementation gate.

### 14.1 Hostile fixtures

| Fixture ID | Predicate/action: single mutation | Expected decision | Expected code | Exact JSON Pointer (container) | Phase | Current |
| --- | --- | --- | --- | --- | --- | --- |
...
| `preconsent-authority-missing` | Remove a required authority from the pre-consent authority set | `HOLD` | `F06-SOURCE-AUTHORITY-MISSING` | `/source_closure/authority_set` | `F06-P5` | `NOT IMPLEMENTED` |
| `preconsent-authority-removed` | Mark a declared authority as removed before acquisition | `REJECT` | `F06-SOURCE-AUTHORITY-REMOVED` | `/source_closure/authority_set` | `F06-P5` | `NOT IMPLEMENTED` |
| `preconsent-authority-reordered` | Permute the declared authority order | `REJECT` | `F06-SOURCE-AUTHORITY-REORDERED` | `/source_closure/authority_set` | `F06-P5` | `NOT IMPLEMENTED` |
| `preconsent-authority-unlisted` | Acquire from an authority absent from the allowlist | `REJECT` | `F06-SOURCE-AUTHORITY-UNLISTED` | `/source_closure/authority_set` | `F06-P5` | `NOT IMPLEMENTED` |
| `acquisition-before-consent` | Acquire a required input before final consent | `REJECT` | `F06-SOURCE-ACQUISITION-PRECONSENT` | `/source_closure/consent_ref` | `F06-P5` | `NOT IMPLEMENTED` |
...
| `duplicate-scope-key` | Duplicate a decision/component semantic scope | `REJECT` | `F06-SCOPE-DUPLICATE` | `/decision_bindings/redistribution/component_paths` | `F06-P1` | `NOT IMPLEMENTED` |
| `path-malformed` | Use absolute, parent, wildcard, and numeric-index syntax | `REJECT` | `F06-INPUT-MALFORMED` | `/scope/component_paths` | `F06-P1` | `NOT IMPLEMENTED` |
...
| `policy-unknown-kind` | Use a policy condition kind outside the closed union | `REJECT` | `F06-POLICY-UNKNOWN` | `/policy/root_condition` | `F06-P1` | `NOT IMPLEMENTED` |
| `policy-unavailable-fact` | Make a required policy fact unavailable | `HOLD` | `F06-POLICY-UNAVAILABLE` | `/policy/root_condition/path` | `F06-P7` | `NOT IMPLEMENTED` |
| `policy-false-predicate` | Supply a false required policy predicate | `REJECT` | `F06-POLICY-PREDICATE-FALSE` | `/policy/root_condition` | `F06-P7` | `NOT IMPLEMENTED` |
| `policy-depth-overflow` | Nest policy conditions beyond the locked depth | `REJECT` | `F06-POLICY-DEPTH-EXCEEDED` | `/policy/root_condition` | `F06-P1` | `NOT IMPLEMENTED` |
| `policy-recursive-alias` | Reuse a recursive condition alias | `REJECT` | `F06-POLICY-CYCLE` | `/policy/root_condition` | `F06-P1` | `NOT IMPLEMENTED` |
| `policy-duplicate-key` | Duplicate a condition semantic key | `REJECT` | `F06-POLICY-DUPLICATE` | `/policy/root_condition/conditions` | `F06-P1` | `NOT IMPLEMENTED` |
| `policy-invalid-digest-reference` | Reference a digest outside the closed policy subject set | `REJECT` | `F06-POLICY-DIGEST-MISMATCH` | `/policy/root_condition/path` | `F06-P1` | `NOT IMPLEMENTED` |
...
| `f02-error-code-laundering` | Map an F-02 `TRUST_FAILURE` rejection to generic warning-success | `REJECT` | `F06-F02-SOURCE-TRUST_FAILURE` | `/f02_findings` | `F06-P3` | `NOT IMPLEMENTED` |
...
| `handshake-capability-mismatch` | Omit one required capability and add an unknown one | `REJECT` | `F06-HANDSHAKE-CAPABILITY` | `/handshake/capabilities` | `F06-P1` | `NOT IMPLEMENTED` |
...
| `source-content-preimage-mismatch` | Change source bytes while retaining content digest | `REJECT` | `F06-SOURCE-CONTENT-MISMATCH` | `/license_bindings/artifact:one/source_offer_ids` | `F06-P6` | `NOT IMPLEMENTED` |
| `source-offer-incomplete` | Remove source digest, size, or availability evidence | `REJECT` | `F06-SOURCE-OFFER-INCOMPLETE` | `/license_bindings/artifact:one/source_offer_ids` | `F06-P7` | `NOT IMPLEMENTED` |
...
| `branding-join-mismatch` | Add an unapproved rendered trademark location | `REJECT` | `F06-BRANDING-UNRESOLVED` | `/decisions/branding_trademark/rendered_locations` | `F06-P7` | `NOT IMPLEMENTED` |
...
| `direct-fetch-cache-missing` | Omit receipt for a direct-device input | `HOLD` | `F06-SOURCE-AUTHORITY-MISSING` | `/source_closure/receipt_refs` | `F06-P5` | `NOT IMPLEMENTED` |
| `recovery-stub-cross-target` | Use a Recovery-stub receipt for another board | `REJECT` | `F06-SCOPE-MISMATCH` | `/source_closure/receipt_refs` | `F06-P5` | `NOT IMPLEMENTED` |
| `package-index-missing` | Omit signed package-index metadata | `HOLD` | `F06-SOURCE-AUTHORITY-MISSING` | `/source_closure/receipt_refs` | `F06-P5` | `NOT IMPLEMENTED` |
| `source-closure-incomplete` | Omit an image, package, or opaque external input | `REJECT` | `F06-SOURCE-AUTHORITY-UNLISTED` | `/inventory/root_node_ids` | `F06-P5` | `NOT IMPLEMENTED` |
| `mutation-network-allowed` | Change network mode from deny-after-final-consent | `REJECT` | `F06-SOURCE-NETWORK-POSTCONSENT` | `/source_closure/network_policy/mode` | `F06-P5` | `NOT IMPLEMENTED` |
...
| `handoff-duplicate-id` | Add a second handoff with an existing identity | `REJECT` | `F06-HANDOFF-DUPLICATE` | `/handoffs` | `F06-P9` | `NOT IMPLEMENTED` |
| `handoff-cycle` | Make a handoff dependency point back to its consumer | `REJECT` | `F06-HANDOFF-CYCLE` | `/handoffs` | `F06-P9` | `NOT IMPLEMENTED` |
...
| `f07-duplicate-writer` | Present two stable writer identities | `REJECT` | `F06-F07-DUPLICATE-WRITER` | `/f07_handoff/writer_binding_refs` | `F06-P9` | `NOT IMPLEMENTED` |
...
| `pseudonym-key-mismatch` | Use a bare hash or wrong HMAC key domain | `REJECT` | `F06-OUTPUT-PSEUDONYM-INVALID` | `/support/support_pseudonyms` | `F06-P8` | `NOT IMPLEMENTED` |
...
| `command-array-invalid` | Encode a glob and conditional as pseudo-shell text | `REJECT` | `F06-COMMAND-ENCODING-INVALID` | `/command_vector` | `F06-P1` | `NOT IMPLEMENTED` |
| `absolute-path-in-command` | Add an absolute host path to argv | `REJECT` | `F06-ABSOLUTE-PATH` | `/command_vector` | `F06-P1` | `NOT IMPLEMENTED` |
...
| `conflict-marker` | Add a merge conflict marker to a tracked artifact | `REJECT` | `F06-CONFLICT-MARKER` | `/artifact_refs` | `F06-P10` | `NOT IMPLEMENTED` |
| `secret-scan` | Put a token or password in a report field | `REJECT` | `F06-SECRET-SCAN` | `/report/artifact_refs` | `F06-P10` | `NOT IMPLEMENTED` |
...
| `bom-input` | Prefix the request with a UTF-8 BOM | `REJECT` | `F06-INPUT-NONCANONICAL` | `/input_content` | `F06-P0` | `NOT IMPLEMENTED` |
| `invalid-utf8-input` | Supply an invalid UTF-8 byte sequence | `REJECT` | `F06-INPUT-MALFORMED` | `/input_content` | `F06-P0` | `NOT IMPLEMENTED` |
| `negative-zero` | Encode negative zero in a numeric field | `REJECT` | `F06-INPUT-NONCANONICAL` | `/scope` | `F06-P2` | `NOT IMPLEMENTED` |
| `request-oversize` | Exceed the locked request byte bound | `REJECT` | `F06-BOUND-EXCEEDED` | `/input_content` | `F06-P0` | `NOT IMPLEMENTED` |
| `evidence-expired` | Use an expired evidence record | `REJECT` | `F06-TRUST-DECISION-EXPIRED` | `/evidence` | `F06-P3` | `NOT IMPLEMENTED` |
| `evidence-revoked` | Use evidence signed by a revoked key | `REJECT` | `F06-F03-TRUST-REJECTED` | `/evidence` | `F06-P3` | `NOT IMPLEMENTED` |
| `evidence-transplant` | Reuse evidence for another candidate scope | `REJECT` | `F06-DECISION-TRANSPLANT` | `/evidence` | `F06-P4` | `NOT IMPLEMENTED` |
| `torn-state-recovery` | Truncate a persisted record during recovery | `HOLD` | `F06-EVENT-TRUNCATED` | `/events` | `F06-P8` | `NOT IMPLEMENTED` |
| `concurrent-consume` | Consume one replay identity concurrently twice | `REJECT` | `F06-EVENT-REPLAY` | `/events` | `F06-P8` | `NOT IMPLEMENTED` |
| `digest-self-reference` | Include the result digest in its own preimage | `REJECT` | `F06-IDENTITY-DIGEST-MISMATCH` | `/result_payload_digest` | `F06-P2` | `NOT IMPLEMENTED` |
...
| `product-integration-to-f06` | Future product-integration owner | F-06 | Required `product_integration_ref` document ID | `payload_digest` and `content_digest` cover the exact integration contract and acceptance bytes with separate domain-separated preimages | Future product-integration schema/binding lock and F-02 manifest binding | Issued/expiry window, revocation epoch, and candidate/channel freshness | Exact project, repository, slice, operation, candidate, board, manifest, artifact set, target account, channel, and integration scope | Required product-integration handoff join before F-07 request assembly | `F06-PRODUCT-INTEGRATION-MISSING` / `/product_integration_ref` / `F06-P9` / `REJECT`; digest/scope/expiry/revocation mismatch uses `F06-SCOPE-MISMATCH` / same pointer / `F06-P4` / `REJECT` | Product-integration owner | G-F06-09 | Future signed product-integration acceptance artifact and recomputed digest report |

## 16. Current state, acceptance census, and residuals

### 16.1 Recomputed design census

The correction contains 129 hostile fixture declarations and 8 positive fixture declarations, one 11-capability handshake set, one closed F-02 upstream code set of 45 values, one closed local F-06 code set of 134 values plus the disjoint 45-value mapped F-02 source namespace, 13 schema/lock files, 31 exact future tracked deliverable paths, and 16 authoritative handoff rows. These are design declarations, not runtime observations. The hostile fixture rows are all `NOT IMPLEMENTED`; no positive fixture has a runtime result.
