...
class ProgramIntegrityTests(unittest.TestCase):
    ...
    def test_current_program_has_closed_census_and_complete_f07_closure(self):
        ...
        self.assertEqual(result["status_counts"], {"DONE": 2, "IN PROGRESS": 17, "TODO": 27, "HUMAN-ONLY BLOCKED": 6})
    ...
    def test_regenerated_lock_accepts_standalone_todo_to_in_progress_transition(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            text = text.replace(
                ...
                "| I-02 | omarchy-mac-installer | Build a signed/notarized native app and equivalent CLI with pinned immutable metadata and resumable pre-mutation artifact acquisition | I-01, F-03, F-06 | IN PROGRESS |",
            )
            text += "| 2026-09-03 | status transition: I-02 from TODO to IN PROGRESS | coordinator evidence |\n"
            ...
            self.assertEqual(result["status_counts"]["IN PROGRESS"], 18)
            self.assertEqual(generated["expected_status_counts"]["IN PROGRESS"], 18)
            self.assertEqual(generated["expected_status_counts"]["TODO"], 26)
    ...
    def test_unknown_program_status_is_rejected(self):
        error = self.run_mutation(self.mutate_text(
            "| I-02 | omarchy-mac-installer | Build a signed/notarized native app and equivalent CLI with pinned immutable metadata and resumable pre-mutation artifact acquisition | I-01, F-03, F-06 | TODO |",
            "| I-02 | omarchy-mac-installer | Build a signed/notarized native app and equivalent CLI with pinned immutable metadata and resumable pre-mutation artifact acquisition | I-01, F-03, F-06 | PLANNED |",
        ))
    ...
    def test_generic_status_mentions_cannot_launder_a_transition(self):
        def generic_transition(path):
            ...
            text += "| 2026-09-03 | I-02 implementation started | generic evidence only |\n"
    ...
    def test_done_transition_is_unauthorized_even_with_exact_evidence(self):
        def done_transition(path):
            text = path.read_text().replace(
                "| I-02 | omarchy-mac-installer | Build a signed/notarized native app and equivalent CLI with pinned immutable metadata and resumable pre-mutation artifact acquisition | I-01, F-03, F-06 | TODO |",
                "| I-02 | omarchy-mac-installer | Build a signed/notarized native app and equivalent CLI with pinned immutable metadata and resumable pre-mutation artifact acquisition | I-01, F-03, F-06 | DONE |",
            )
            text += "| 2026-09-03 | status transition: I-02 from TODO to DONE | coordinator evidence |\n"
            path.write_text(text)
